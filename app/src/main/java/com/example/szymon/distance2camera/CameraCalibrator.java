package com.example.szymon.distance2camera;
...
public class CameraCalibrator {
    ...
    private int mFlags;
    private double mRms;
    private double mSquareSize = 1.4;
    ...
    public void calibrate() {
        ...
        Mat tvec = tvecs.get(0);
        for (int i = 0 ; i < 3; i++)
            translation[i] = (float)tvec.get(i, 0)[0];

        Log.i(TAG, "Translation " + tvecs.get(0).dump());




          //  x = String.valueOf(translation[0]);
         //   y = String.valueOf(translation[1]);
          //  z = String.valueOf(translation[2]);

    }
public void calculate_distance(MatOfPoint3f objectPoints,MatOfPoint2f imagePoints,Mat cameraMatrix,MatOfDouble distCoff)
{
    Mat rvec_new = new Mat();
    Mat tvec_new = new Mat();
    Calib3d.solvePnPRansac(objectPoints,imagePoints,cameraMatrix,distCoff, rvec_new,tvec_new);
    float[] translation = new float[3];
    for (int i = 0 ; i < 3; i++) {
        translation[i] = (float) tvec_new.get(i, 0)[0];
    }
    x = String.valueOf(translation[0]);
    y = String.valueOf(translation[1]);
    z = String.valueOf(translation[2]);
    ...
    private void renderFrame(Mat rgbaFrame) {
        ...
        if(mPatternWasFound && z!="null")
        {
            for (int i = 0; i < objectPoints.size(); i++) {

               MatOfPoint3f points = new MatOfPoint3f(objectPoints.get(i));

               calculate_distance(points, mCorners, mCameraMatrix, distortionCoefficients);
            }
      ...
      //  Imgproc.putText(rgbaFrame, "XYZ: " + tvecs.get(0).dump(),  new Point(rgbaFrame.cols() / 10, rgbaFrame.rows()),
         //       Core.FONT_HERSHEY_SIMPLEX, 1.0, new Scalar(255, 255, 0));
        Imgproc.putText(rgbaFrame,"Distance to marker " + z+"cm",  new Point(rgbaFrame.cols() / 20, rgbaFrame.rows()-3),
                Core.FONT_ITALIC, 1.0, new Scalar(0, 0, 255),3);
    }
